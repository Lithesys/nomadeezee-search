...
export async function buildServer(
  ...
  const app = Fastify({
    logger: {
      ...
      redact: [
        ...
        "req.headers.x-search-webhook-secret",
        "req.headers.x-search-admin-secret",
      ],
    },
  });
  ...
  >();
  const typesense = createTypesense(env);
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    if (!token) return;
    try {
      request.identity = await verify(token);
    } catch {
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    if (!bucket || bucket.resetAt <= now)
      anonymousBuckets.set(key, { count: 1, resetAt: now + 60_000 });
    else if (bucket.count >= env.SEARCH_PUBLIC_RPM)
  ...
  void ensureCollection(typesense, env).catch((error) =>
    app.log.warn(
      {
        error: error instanceof Error ? error.message : "Typesense unavailable",
      },
      "Typesense collection initialization deferred",
    ),
  );
